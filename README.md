...
- `hop workspace` persists a root-level `workspaces.json` index with per-workspace hydration/cursor state, can discover account-visible D1 cloud codebases with local readiness when credentials allow it, falls back to the configured codebase under scoped device tokens, can attach a cloud codebase into the Workspace Root as metadata-only, list visible cloud files, hydrate one file, safely hydrate the full workspace through refresh, and dehydrate clean workspaces back to metadata-only state.
