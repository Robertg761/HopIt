...
import { downloadPlatforms } from '@/lib/download-platforms'
...
export default async function DownloadPage() {
  ...
  return (
    <PublicShell signedIn={signedIn}>
          ...
          <div className="overflow-hidden rounded-2xl border border-[#30363d] bg-[#010409] shadow-[0_28px_90px_rgba(0,0,0,.38)]">
            ...
            <div className="p-5 sm:p-7">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-[#8b949e]">
                <span className="inline-flex items-center gap-1.5"><ShieldCheck className="size-3.5 text-[#3fb950]" aria-hidden /> Checksum verified</span>
                <span className="inline-flex items-center gap-1.5"><Download className="size-3.5 text-[#58a6ff]" aria-hidden /> Runtime included</span>
                <ExternalLink className="rounded-sm text-[#58a6ff] hover:underline" href="/install">Review installer</ExternalLink>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="manual-downloads" className="mx-auto w-full max-w-[1180px] scroll-mt-24 px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          ...
          <div className="grid gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2">
            {downloadPlatforms.map((platform) => (
              <article key={platform.architecture} className="bg-card p-5 sm:p-6">
                <span className="grid size-10 place-items-center rounded-lg border border-border bg-muted/55"><Laptop className="size-5 text-hop" aria-hidden /></span>
      ...
      <section className="mx-auto w-full max-w-[1180px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="grid overflow-hidden rounded-2xl border border-border lg:grid-cols-[1.15fr_.85fr]">
          <div className="p-6 sm:p-9">
            ...
            <DownloadDashboardButton signedIn={signedIn} />
          </div>
          <div className="border-t border-border bg-[#0d1117] p-6 text-[#f0f6fc] sm:p-9 lg:border-l lg:border-t-0">
            ...
            <ul className="mt-5 space-y-3 text-sm text-[#a1a9b3]">
              <li className="flex gap-2.5"><Check className="mt-0.5 size-4 shrink-0 text-[#3fb950]" aria-hidden /> Use the same HopIt account on every device.</li>
              <li className="flex gap-2.5"><Check className="mt-0.5 size-4 shrink-0 text-[#3fb950]" aria-hidden /> Each device is approved separately.</li>
              <li className="flex gap-2.5"><Check className="mt-0.5 size-4 shrink-0 text-[#3fb950]" aria-hidden /> Reads and exports remain available at quota limits.</li>
            </ul>
          </div>
        </div>
      </section>
...
function SetupStep({ icon: Icon, title, detail }: { icon: typeof Terminal; title: string; detail: string }) {
  return (
    <article className="relative border-border px-1 py-6 lg:border-r lg:px-8 lg:py-2 last:lg:border-r-0">
      <span className="grid size-10 place-items-center rounded-lg border border-border bg-background"><Icon className="size-5 text-hop" aria-hidden /></span>
