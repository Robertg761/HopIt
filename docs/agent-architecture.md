...
The current implementation has a production-profile managed workspace path, a root-level `workspaces.json` index, D1 account-visible codebase discovery merged with local attach/readiness state when credentials allow it, scoped-token configured-codebase fallback, dashboard-driven first-run setup/metadata-only attach, hydration/materialized-revision status, visible-file metadata listing, single-file hydration, metadata-only dehydrate state, and a remote cursor exposed through `hop status`. Solid v1 still needs automatic account bootstrap, richer hydrate onboarding, automatic lazy materialization policy, and production-grade event delivery before claiming the "install and boom" experience.
...
- Add root-level codebase discovery and attach/hydrate state. D1 account-visible discovery, dashboard first-run setup/metadata-only attach, and hydration/readiness state are in place; automatic account bootstrap and richer hydrate onboarding remain.
