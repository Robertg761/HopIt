...
| Cloud graph | Cloudflare D1 | Cloudflare dashboard/API, `cloudflare/d1/schema.sql`, `cloudflare/d1/api-worker.js`, `HOPIT_D1_*` | Stores graph metadata, file metadata, account sync, action jobs, events, members, invitations, scoped sessions, and trusted-device/key metadata on a free-first backend. |
...
- Build the GitHub-like product layer around HopIt's own model: code browsing, diffs, review comments, history, permissions, and invitations.
...
- Hosted workspace commands are intentionally disabled; local workspace commands run through the local agent. Hosted collaboration/member APIs exist behind Clerk product auth.
...
- The dashboard now has first-project device approval, a four-step Workspace Root checklist, a first read-only code browser, plus member/invite and key-grant status surfaces. Web compare UI wiring, key approval/rotation UX, and a successful clean-workspace live push-apply proof remain future work.
