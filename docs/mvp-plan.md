...
- `/api/agent/command` exposes whitelisted local actions for sync, refresh, recover, review, merge, first-run Workspace Root setup, Workspace Root attach, safe full hydrate, file-level hydrate/pin/free-space actions, and metadata-only dehydrate. Local dashboard server routes merge `~/.config/hopit/production.env` under the Next.js process env, and command execution uses `--profile production` when installed-agent paths are configured, so D1 credentials and Workspace Root paths stay aligned. Hosted D1-backed deployments can read status but still require the local agent for workspace commands.
- `hop workspace` persists a root-level index keyed by codebase and concrete workspace path; D1 account-visible discovery now merges cloud codebase heads with local attach/readiness state when credentials allow it, scoped device tokens fall back to the configured codebase, automatic verified-owner bootstrap can claim migrated `local-owner` codebases, dashboard-driven first-run setup/metadata-only attach can bind a cloud codebase into the Workspace Root through the local agent, and hydrate, hydrate-path, pin/unpin, prune, refresh, dehydrate, and sync update the cache/hydration/materialized revision state that status and remote-pull use.
...
1. Finish the HopIt Workspace Root product contract: editor/tool demand hydration, automatic cache pruning policy, and broader blocked/conflicted UI detail on top of the current D1 account-visible discovery, automatic account bootstrap, dashboard-driven setup/metadata-only attach/hydrate/dehydrate controls, per-file cache states, and path-level hydrate/pin/prune primitives.
...
- Create a durable workspace-root configuration and setup flow. The root index, account-visible discovery, automatic account bootstrap, and dashboard first-run setup/metadata-only attach/hydrate/dehydrate/file-cache actions exist.
- Represent codebase folders, hydration state, local cache state, and remote event cursors explicitly. Codebase/workspace entries, materialized revision cursors, metadata-only state, partial path hydration, per-file cache states, pin/unpin, prune events, and metadata-first policy exist; editor/tool demand hydration remains.
