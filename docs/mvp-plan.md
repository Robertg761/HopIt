...
- `/api/agent/command` exposes whitelisted local actions for sync, refresh, recover, review, merge, first-run Workspace Root setup, Workspace Root attach, safe full hydrate, and metadata-only dehydrate. Local dashboard server routes merge `~/.config/hopit/production.env` under the Next.js process env, and command execution uses `--profile production` when installed-agent paths are configured, so D1 credentials and Workspace Root paths stay aligned. Hosted D1-backed deployments can read status but still require the local agent for workspace commands.
- `hop workspace` persists a root-level index keyed by codebase and concrete workspace path; D1 account-visible discovery now merges cloud codebase heads with local attach/readiness state when credentials allow it, scoped device tokens fall back to the configured codebase, automatic verified-owner bootstrap can claim migrated `local-owner` codebases, dashboard-driven first-run setup/metadata-only attach can bind a cloud codebase into the Workspace Root through the local agent, and hydrate, refresh, dehydrate, and sync update the materialized revision cursor that status and remote-pull use.
...
1. Finish the HopIt Workspace Root product contract: richer per-file cache metadata, editor/tool demand hydration, cache pruning policy, and broader blocked/conflicted UI detail on top of the current D1 account-visible discovery, automatic account bootstrap, dashboard-driven setup/metadata-only attach/hydrate/dehydrate controls, and single-file hydrate primitives.
...
- Create a durable workspace-root configuration and setup flow. The root index, account-visible discovery, automatic account bootstrap, and dashboard first-run setup/metadata-only attach/hydrate/dehydrate actions exist.
- Represent codebase folders, hydration state, local cache state, and remote event cursors explicitly. Codebase/workspace entries, materialized revision cursors, metadata-only state, partial single-file hydration, and metadata-first policy exist; richer per-file cache policy remains.
