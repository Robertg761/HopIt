...
**GR-E3: releases push an annotated git tag to the mirror** (decisions §8/§9). `createRelease` (`packages/agent/src/commands/release.js`) calls the new `runMirrorTagRelease` (`packages/agent/src/commands/mirror.js`) right after the release row is inserted: it resolves the mirror commit for the release's pinned Main revision, pushes an annotated tag whose message is the release name plus its notes, and returns `null` (doing nothing, erroring nothing) when no mirror is configured. Tagging failure never blocks the release — it surfaces as a `git.mirror_tag_failed` event, the same "notify, never block the product action" rule GR-E2 uses for mirror-on-merge. Every mirror commit now carries a `Main-Revision: <n>` git trailer so a release can find its commit from the pushed history itself rather than from per-machine local bookkeeping; `assertSafeGitTagName` delegates release-name validation to `git check-ref-format`. Comment-only touches to `packages/backend-d1/src/schema.js` and `releases-store.js`; **no schema change and no new migration**.

**GR-X1: end-to-end adversarial suite.** New `packages/agent/test/git-replacement-e2e.test.js` — five scenarios chaining the whole pipeline, two devices sharing one cloud graph driven through the real `hop` CLI, each ending in a recursive path→sha256 compare across both devices, the cloud graph, and (scenario 1) a checkout of the git mirror. (1) Offline divergence → reconnect classification → `hop conflicts resolve` → `hop propose --merge` → merge queue lands it → `hop mirror-sync` advances the bare repo by exactly one commit, with no `.private/` path in the git tree. (2) A `hop watch` process SIGKILLed mid-session, edits made while it was down (modify, create, delete), restart → the startup diff-scan synthesizes the entries, replays them with `diverged: 0`, and both devices converge byte-identically. (3) One session containing 301 writes under `node_modules/`, a planted AWS key in a source file, and an over-threshold file → 0 derived paths journaled or synced, exactly 1 `secret.suspected` (the derived look-alike is never scanned), exactly 1 `file.large`, full content synced, every legitimate source file intact. (4) A stale editor buffer saved over a refreshed file → save-clobber divergence, Main's content still recoverable, zero silent reverts either direction, then resolved through `hop conflicts resolve --keep cloud`. (5) A loopback D1 worker severed at the socket mid-edit → `hop status` stays up and reports the outage, nothing reaches the cloud, and reconnecting converges byte-identically with no lost writes.

Two real seams the chained scenarios exposed, both fixed:

- `sync.save_clobber_diverged` (GR-F2) was emitted but never reached `deriveOpenDivergences`, which only recognized `journal.reconnect_diverged`. A stale-buffer clobber was therefore invisible to `hop conflicts` and to `status.divergences`, contradicting GR-F2's own acceptance ("surface via GR-A3"). `packages/agent/src/reconnect.js` now normalizes both event shapes into one open set (`divergenceOpeningEvents`), so a save-side divergence lists and resolves through exactly the same path as a reconnect divergence.
- `hop status` died when the cloud was unreachable — a remote backend throws a transport error where a not-yet-initialized local graph returns `null` — which is precisely when status is the surface you reach for. `packages/agent/src/status-state.js` now guards that read, degrades to the already-supported `null` graph, and reports `cloudReachable: false` with `cloudReadError`.

Also fixed a pre-existing test-harness flake unrelated to either task: the polling `readNdjson` helpers in `agent-cli.test.js` and `sync-coalescing.test.js` read journal/event files while they are still being appended to, and `JSON.parse` on a torn trailing line failed the run under load. Only the trailing fragment is now parsed defensively; no assertion was weakened.

**Known, not fixed:** against a *fully* unreachable cloud the journal does not accumulate. `performSyncOnce` reads the cloud graph before it can plan (and therefore journal) anything, so a hard outage fails at that read and no entry is written; what keeps the writes safe is the workspace plus GR-A4's startup diff-scan synthesizing them at reconnect. Scenario 5 asserts that real behavior. Making the journal genuinely write-ahead of the cloud read is a larger change to the sync path and is left as a separate task.

Evidence: `npm run agent:test` — 532 tests, 525 pass, 0 fail, 5 cancelled (the same environment-only Linux cancellations as baseline), 2 skipped; baseline before this wave was 522/515 with the same 5 cancellations, so the growth is exactly GR-E3's 5 tests and GR-X1's 5 scenarios with zero regressions. `npm run test:worker` 89/89. `npm run test:web` 250/250 across 45 files. `npm run lint`, `npm run typecheck:agent`, `npm run build`, `node packages/agent/src/cli.js --help` all clean. The new suite is picked up by `npm run agent:test`'s glob and therefore by `test:all`; it was confirmed running inside the full battery, not only standalone.

## 2026-07-23 GR-H1: Periodic Full Diff-Scan (Missed-Watcher-Event Healing)

Implemented the first git-replacement task from [docs/git-replacement-implementation-plan.md](git-replacement-implementation-plan.md) (§ Track H): decisions doc §12 records that missed watcher events are assumed, not exceptional, and should heal on a periodic full workspace diff-scan.

- `createWorkspaceScanScheduler` in `packages/agent/src/watch.js` runs alongside the fs watcher inside `watchWorkspace`, independent of both the watcher's own health (healthy, degraded to polling, or unavailable) and the separate 5-min cloud graph-head reconciliation (which only covers the cloud side). Each tick re-walks the workspace with the same cheap stat-based `snapshotWorkspace` comparison the degraded-polling fallback already used, excludes derived paths via the existing `shouldSkipWorkspacePath` (GR-C1 already had this in place from the `generatedWorkspaceDirectories` skip list), and on drift calls the normal watch-sync scheduler exactly like a live watcher event — so a missed write heals within one scan interval.
- Default interval is 10 minutes; configurable via `--scan-interval-ms` / `HOPIT_SCAN_INTERVAL_MS`, forwarded through `runtimeArgsFromOptions` to the spawned service process like the existing auto-prune/remote-pull cadence options. New events: `watch.scan_started`, `watch.scan_completed` (carries `durationMs`/`changed`), `watch.scan_healed`, `watch.scan_failed`.
- New tests in `packages/agent/test/watch-schedulers.test.js` (4 → 10 tests in that file) cover: cadence validation, env/CLI/service-arg forwarding, a missed-write healing on the next scan (watcher never notified, only the periodic scan observes it), derived-path exclusion staying silent across repeated churn, and a 5,000-file synthetic workspace scan completing in ~160-700ms (generous 20s bound asserted).
- Full battery run on this branch: `agent:test` 334 pass / 0 fail / 2 skipped / 5 cancelled (env-only, same as baseline) — up from the 328-pass baseline by exactly the 6 new tests; `test:worker` 87/87; `test:web` 211/211 across 37 files; lint, typecheck:agent, build, and `hop --help` all clean. Zero regressions vs the recorded 2026-07-23 baseline.
...
## 2026-07-23 GR-H3: One Agent Per Workspace (Exclusive Lock)

Implemented the git-replacement plan's GR-H3 task (decisions doc §12: "One agent per workspace, enforced"). `watchWorkspace` now takes an exclusive lock on the workspace folder (`<workspace>/.hopit-agent/lock.json`, `packages/agent/src/workspace-lock.js`) before touching the cloud service or journal. The lock records the holder's pid, hostname, codebase id, and start time using an atomic `fs.open(path, 'wx')` create, so a second `hop watch`, a second `hop service run`, or any other agent attaching to the same folder — regardless of profile, state-root, or session — refuses to start non-zero and names the current holder instead of racing the existing watcher. A lock left behind by a process that has since died is detected via a same-host pid liveness check and taken over automatically; a lock recorded from a different hostname is never treated as stale. The lock releases on clean shutdown (`hop watch` now handles SIGINT/SIGTERM to close gracefully instead of relying on Node's default abrupt termination; `service.js` awaits the watch handle's close) and on any startup failure after the lock was acquired.

Fixed a regression discovered while adding this: `.hopit-agent/` inside a workspace folder was not excluded from `shouldSkipWorkspacePath`/`shouldSkipLiteralMirrorPath` (only the unrelated `.hopit/` metadata dir was), so the new lockfile was briefly detected as an unjournaled local change and broke refresh-safety in two-device tests. Both skip-path helpers now exclude `.hopit-agent/` alongside `.hopit/`.

New tests: `packages/agent/test/workspace-lock.test.js` (9 tests) — unit coverage of acquire/release, live-holder refusal with the holder identified, dead-pid stale-lock takeover, different-host locks never treated as stale, release only removing a lock it still owns, a 20/20-iteration concurrent-acquire race (exactly one winner every iteration), and CLI-level integration tests spawning real `hop watch` processes for second-attach refusal, clean-shutdown release, and stale-lock takeover.

Verification (this worktree, 2026-07-23): `npm run agent:test` 337 pass / 0 fail / 5 cancelled (environment-only, same five as baseline) / 2 skipped (344 total, +9 vs the 335-test baseline, all new); `npm run test:worker` 87/87; `npm run test:web` 211/211 across 37 files; `npm run lint`, `npm run typecheck:agent`, `npm run build`, and `node packages/agent/src/cli.js --help` all clean. Zero regressions vs the recorded baseline.
## 2026-07-23 GR-C1: Derived-Path Classification (Never Journaled, Never Synced)

Implemented GR-C1 from [docs/git-replacement-implementation-plan.md](git-replacement-implementation-plan.md), per [docs/git-replacement-decisions-2026-07.md](git-replacement-decisions-2026-07.md) §6: generated/dependency directories are a classification distinct from both privacy zones and any ignore file (there is no config file in the repo). They are never watched-through, journaled, synced, or counted in presence, and each device regenerates its own.

- `packages/agent/src/constants.js` gained `curatedDerivedPathRules`: the curated built-in list (`node_modules`, `.venv`, `venv`, `target`, `dist`, `build`, `.next`, `__pycache__`, `.cache`, `.turbo`, `.gradle`, `vendor/bundle`, plus the pre-existing `generatedWorkspaceDirectories` entries it replaces). Rules are either a bare segment name (matches anywhere) or a `/`-joined root-anchored subtree (`vendor/bundle` does not derive a plain top-level `vendor/`).
- `isDerivedWorkspacePath` in `packages/agent/src/workspace-manifest.js` is the single classification function, wired into `shouldSkipWorkspacePath` (the one function every workspace scan funnels through). It layers per-codebase `{ add, remove }` overrides on top of the curated list.
- Overrides persist in a new `codebase_settings.derived_path_overrides` column (schema.js + schema.sql + `cloudflare/d1/migrations/2026-07-23-derived-path-overrides.sql`, not applied to live D1), read/written via `readCodebaseSettings`/`setDerivedPathOverrides` in `@hopit/backend-d1`'s episodes-store.js (same row trail-summaries already uses) and mirrored in the local-dev `FixtureJsonCloudGraphService`.
- New CLI: `hop derived list|add <path>|remove <path>`. Sync-triggering flows (`syncOnce`, `refreshWorkspace`, `recoverJournal`) resolve overrides once per call via `withDerivedPathOverrides` (never per file), so classification itself stays a pure, in-memory, sub-millisecond check in the watch/scan hot path.
- `hop status` reports currently-present derived roots under `workspace.derivedPaths.excludedRoots` via `listDerivedWorkspaceRoots`, which never descends into a derived directory it finds.
- A flag-gated (`NEXT_PUBLIC_DERIVED_PATHS_SETTINGS`), read-only dashboard card was added (`/api/codebases/derived-paths` + `DerivedPathsCard`); mutation stays CLI-only, matching this settings page's existing "managed via CLI" convention.
- New tests: `packages/agent/test/derived-paths.test.js` (curated/override classification, the 500-derived-file + 3-source-file simulation journaling exactly 3 entries, override sync/un-sync behavior, `hop derived` round trip, `hop status` excluded-roots listing, and a <1ms-per-event classification benchmark), plus scoped-SQL policy coverage in `cloudflare/d1/api-worker.test.js` and a route test in `src/app/api/codebases/derived-paths/route.test.ts`.
- Verified against the 2026-07-23 baseline with zero regressions: agent 328→340 pass (same 5 environment-only cancellations, same 2 skipped), worker 87→89 pass, web 211→219 pass, lint/typecheck/build/CLI boot all clean.
...
### 2026-07-24 GR-F1: per-file refresh — untouched files apply, dirty files delay + flag

`refreshWorkspace` (`packages/agent/src/commands/sync.js`, used by `hop refresh`, the one-shot `hop remote-pull`, and the periodic remote-pull reconciliation loop in `packages/agent/src/watch.js`) no longer blocks the entire workspace refresh just because one file carries unjournaled local drift (decisions doc §10). It now splits the apply per path: paths with no local edits materialize immediately (`materializeCloudToWorkspace`'s new `withheldPaths` parameter), while paths with genuine drift against the current cloud graph are withheld — neither overwritten nor deleted — and flagged with a dedicated `file_withheld_local_edits` reason distinct from the existing whole-workspace `workspace_has_unjournaled_changes` block. The whole-workspace outer gates (pending/failed journal entries, an untrustworthy content manifest, the mass-delete guard) are unchanged and still block everything, as decisions §10 requires.

A withheld path never advances the indexed `lastMaterializedRevision` past cloud head (`workspace-index.js`'s `workspaceIndexEntryFromCloud` gained a `metadata.materializedRevision` override for this), so the "already at head" reconciliation shortcut in `remoteRefreshDecision` keeps re-attempting the withheld path on every cycle until the local edit resolves (matches cloud byte-for-byte, or gets journaled/synced) — at which point the existing stale-manifest self-heal/exoneration machinery reconciles it normally. `status.workspace.withheldRefresh` ("Main changed under you") is computed live against the current cloud graph on every status call via a new disk-scoped helper (`withheldRefreshPathsFromDisk`) that only reads bytes for paths that are also present in cloud — it does **not** do a full directory walk/read, preserving the existing "status never reads untracked file bytes" guarantee.

`remoteRefreshDecision` is shared with `remote-push.js` (live push delivery), which is out of scope for this task and deliberately keeps the old whole-workspace block on any local drift; the per-file relaxation is opt-in via a new `context.allowPerFileWithholding` flag that only the remote-pull call sites set.

Tests: `packages/agent/test/agent-cli.test.js` — new `GR-F1 per-file refresh: untouched files apply, a dirty file is withheld and flagged, then applies once resolved` two-device test (Main merges an untouched file and a file device B has a dirty local edit to; the untouched file applies within one refresh cycle, the dirty file is withheld and flagged in both `refresh.complete` and `status`, and resolving it lets a later refresh fully converge without the indexed revision ever having stranded ahead). `packages/agent/test/workspace-manifest-heal.test.js` — `refreshWorkspace` now withholds+flags a single genuinely-dirty file instead of throwing. Existing dirty-workspace remote-pull/status coverage updated for the new reason/shape (`workspace_has_unjournaled_changes` → `file_withheld_local_edits` where the only changed cloud path is the dirty one).

### 2026-07-24 GR-F2: save-side clobber detection

`packages/agent/src/save-clobber.js` implements the save-side clobber protection from decisions §10: since the agent only ever sees disk, never an unsaved editor buffer, the check runs on the next local save rather than on the refresh itself. `refreshWorkspace` records a per-path "pending refresh" (hash + revision) in a small writer ledger (`<journal-file>.writer-ledger.json`, colocated with the journal it protects) for every path `materializeCloudToWorkspace` actually changed. `performSyncOnce` consults that ledger for every disk write about to be journaled and, when a pending refresh exists, reuses GR-A1's `classifyReconnectEntry` (`packages/agent/src/reconnect.js`) to classify the save exactly like a reconnect classification: identical-to-refresh or content that contains (builds on) the refreshed text auto-resolves as a clean edit; anything else is a genuine divergence. Diverged saves are never journaled or committed — Main's cloud content is left exactly as it was, never a silent revert of either side — and instead emit `sync.save_clobber_diverged` (path, refreshed hash/revision, local hash) so a future surface (`hop conflicts`, GR-A3) can resolve them. The local file's bytes on disk are untouched in both the clean and diverged cases; only the push to cloud is withheld for a diverged save.

Tests: `packages/agent/test/save-clobber.test.js` — unit coverage of `classifySaveAgainstRefresh` (no pending refresh, identical-content auto-resolve, builds-on-refresh auto-resolve, genuine divergence), `buildsOnRefreshedContent` (empty-refresh and binary-content edge cases), and writer-ledger read/write/mark/clear round-trips; two end-to-end `hop refresh` + `hop sync-once` scenarios over a shared cloud.json between two device workspaces prove the accept criteria directly: a stale-buffer save that ignores a refresh opens `sync.save_clobber_diverged` and leaves Main's cloud content and the local device's disk bytes both untouched (zero silent reverts either direction), while a save that incorporates the refreshed content commits normally with no false-positive divergence.

Known limitation: local deletes racing a pending refresh are out of scope for this task (F2's accept criteria cover saves/writes only); the ledger clears on a committed delete but does not itself classify delete-vs-refresh. Formal divergence persistence/resolution surfaces (GR-A2, GR-A3) have not landed yet, so a diverged save currently stays withheld (never committed, event-flagged) rather than resolvable from a `hop conflicts` command.

### 2026-07-24 GR-G1: idle dehydration with a user-tunable window + safety invariants

Auto-prune (idle dehydration) is now default-on instead of opt-in: `createAutoPruneScheduler` (`watch.js`) starts unless a caller explicitly passes `--no-auto-prune` or sets `HOPIT_AUTO_PRUNE=0`; the default idle window stays 7 days (`--auto-prune-inactive-ms` / `HOPIT_AUTO_PRUNE_INACTIVE_MS`). v1 dehydration remains dematerialization of the whole codebase back to metadata-only state via the existing `dehydrateWorkspace` / `pruneWorkspaceCache` machinery — no per-file placeholder stubs — per the Track G intro's resolution of the WS7b design conflict; that resolution is now also recorded in `docs/agent-architecture.md`.

New in this pass:

- **Disk-pressure acceleration**: `diskPressureAcceleratedInactiveMs` (`watch.js`) shortens the idle window when the workspace device's free space drops below an absolute floor (5 GB default) or a free-fraction floor (10% default), by a fixed acceleration factor (25% of the configured window, floored at the scheduler's 60s minimum cadence). A pure function so tests inject synthetic disk stats instead of filling a real disk; wired into the scheduler via `statWorkspaceDisk` (`fs.statfs`) with an injectable override for tests. An accelerated run emits `cache.auto_prune_disk_pressure` before the shortened prune executes.
- **Journal-path guard**: `pruneWorkspaceCache` now refuses any eviction candidate whose absolute path resolves onto the configured journal or events ndjson file (`journal_path` skip reason) — defense in depth on top of the structural fact that the journal is never part of the cloud file graph, satisfying "the journal itself is last-sacrificed."
- **Harness-time clock injection**: `pruneWorkspaceCache` now accepts `options.now` (mirroring the existing convention in `applyJournalEntryToCloud`), so tests simulate the idle window elapsing without depending on wall time.
- **Status surface**: `hop status` exposes `autoPrune: { enabled, lastStarted, lastSkipped, lastFailed, lastDiskPressure, lastEvicted, latestEvent }`, sourced from the existing `cache.auto_prune_*` / `cache.evicted` event log (same pattern as `secretScan`/`watch`).
- `pruneWorkspaceCache` now returns its result object (previously it only printed it), so callers (tests, future status code) can read `skipped`/`removedPaths` without parsing stdout.

Per-folder "keep on this device" pins (`hop workspace pin`/`unpin`) and the unacknowledged-journal-write refusal (`journal_pending`/`journal_failed` skip reasons) already existed in `pruneWorkspaceCache` pre-Wave-1 and needed no change; this pass added test coverage proving both hold under the harness-time idle-window scenario the plan's accept criteria describe.

Tests: `packages/agent/test/idle-dehydration.test.js` (new) — an idle codebase dehydrates to metadata-only (zero on-disk bytes) once its window elapses and not before; a pinned path survives that same eviction while unpinned paths do not; a path with a pending journal entry refuses eviction; an adversarial cloud path that resolves onto the real journal file is refused (`journal_path`) and the real journal file is left untouched; re-opening a dehydrated codebase re-materializes every file byte-identical (SHA-256 compared). `packages/agent/test/watch-schedulers.test.js` (extended) — auto-prune defaults on with no flag at both the CLI-options and scheduler level; `--no-auto-prune`/`HOPIT_AUTO_PRUNE=0` still disable it and the disable forwards to the service child process; disk-pressure acceleration is exercised as a pure function (ample space is a no-op, low absolute free bytes and low free fraction both shorten the window, the floor never goes below the scheduler minimum, missing disk stats are a no-op) and end-to-end through the scheduler (a low-free-space `statDisk` narrows the `inactive-ms` handed to `pruneWorkspaceCache` and emits `cache.auto_prune_disk_pressure`; ample free space does neither).

Metrics demonstrated: unacked-write eviction attempts = 0 succeed (the pending-journal-entry test asserts the file remains on disk and is absent from `removedPaths`); dehydrated codebase's on-disk footprint is metadata-only (a dedicated test walks the workspace tree after eviction and asserts total bytes on disk = 0); full agent battery green with zero regressions vs the post-Wave-1 baseline (417 total / 410 pass / 0 fail / 2 skipped / 5 cancelled) — this branch: 434 total / 427 pass / 0 fail / 2 skipped / 5 cancelled, the same 5 environment-only Linux cancellations as baseline (periodic head reconciliation transient-fault/retry-budget tests, reconnect fallback-poll classification tests). `npm run test:worker` 89/89 pass, `npm run test:web` 219/219 pass, `npm run lint`/`npm run typecheck:agent`/`npm run build`/`node packages/agent/src/cli.js --help` all clean.

Out of scope for this task (reported, not fixed): the plan's "settings plumbing" for a dashboard-configurable per-codebase idle window (mirroring GR-G2's `codebase_settings.large_file_threshold_bytes` pattern) was not added — GR-G1's own Files list scopes this to `hydrate.js`/`watch.js`/CLI option plumbing/status surfaces, and the CLI flag + env var already give per-invocation control; a D1-backed per-codebase override is a natural follow-up but would be new schema surface outside this task's listed files.

### 2026-07-24 GR-E2: mirror automation on merge

Builds on GR-E1 (`hop mirror-sync`) to close the automation loop from decisions doc §8: `hop mirror-set-remote <url> [--branch <name>] [--deploy-key <path>]` (`packages/agent/src/commands/mirror.js`) persists the mirror destination server-side in `codebase_settings` (new `mirror_remote_url`/`mirror_branch`/`mirror_deploy_key_ciphertext`/`mirror_deploy_key_metadata` columns, migration `cloudflare/d1/migrations/2026-07-24-mirror-remote-config.sql`) so a hosted runner with no prior local `mirror-state.json` still knows where to push on its first invocation. A deploy key is always client-encrypted before it leaves the process — reusing `@hopit/core/crypto`'s `encryptClientPayload`/`decryptClientPayload` AES-256-GCM envelope (the same one already used for `.private/env/`, under a synthetic `.private/env/mirror-deploy-key` path so it inherits the identical "secrets" privacy zone and never-unencrypted rule); setting a deploy key without a configured `HOPIT_CLIENT_ENCRYPTION_KEY` is rejected outright rather than ever persisting plaintext.

A successful `hop merge` (`mergeChangeSet`, `packages/agent/src/commands/sync.js`) best-effort enqueues one `mirror` `action_job` (`enqueueMirrorSyncJob`, `packages/backend-d1/src/actions.js`) when a remote is configured for that codebase — the merge itself always commits first and a read/enqueue failure is only journaled (`mirror.enqueue_failed`), never surfaced as a merge failure. `packages/actions-runner/src/runner.js` recognizes the `mirror` job kind and runs `hop mirror-sync` directly against the codebase's own configured remote/branch/deploy-key (no workspace checkout or `npm ci` — the mirror never reads from a runner-local git clone, only from cloud content hashes). A mirror-push failure surfaces as a `mirror.failed` dashboard notification (`completeActionJob`) and never blocks, delays, or unwinds the merge that triggered it.
...
### 2026-07-24 GR-D2: dashboard/menu-bar secret flag

Builds on GR-D1's warn-only scanner (`packages/agent/src/secret-scan.js`, `secret.suspected` event) to give the owner an actual "rotate, don't redact" flag on every surface (decisions doc §7), without any new scanning logic.

The event → notification round-trip needed no new plumbing beyond `appendEvent` itself: `emit()` (`packages/agent/src/io.js`) already pushes every event to D1 via `appendRemoteEvent` when a D1 backend is configured, so `packages/backend-d1/src/graph.js`'s `appendEvent` now calls the existing `createNotification` helper whenever `event === 'secret.suspected'`, producing a `secret.suspected`-kind row (title `Possible secret in <path>`, body carrying the rotation guidance, `href` into the codebase's Activity page) the moment the agent event lands — same mechanism review-thread/review-decision notifications already use. New helpers `secretFindingTitle`/`secretFindingBody`/`secretFindingHref` in `packages/backend-d1/src/helpers/collaboration.js` mirror the existing `reviewDecisionTitle`/`reviewHref` pattern.

Web dashboard (`src/components/features/activity/`): `NotificationsCard` is the durable, per-finding-dismissible surface — its existing "Mark read" button doubles as dismiss with no new state needed, and a `secret.suspected` row gets a danger-toned status dot plus a "Rotate, don't redact" badge. `EventLedger` (the raw agent-event feed, filterable All/Sync/Review/Devices/Privacy) now describes `secret.suspected` as "possible secret in `<path>` (N findings) -- rotate it, don't redact it" with an escalated `blocked` tone, and surfaces under the Privacy filter alongside other privacy-zone events. `mapRecentEvents` (`src/lib/client/agent-status/mappers.ts`) carries the describe/tone logic both consume.

Desktop: `packages/desktop/src/lib/state.js`'s `projectStateFromProbe` treats any open `status.secretScan.suspectedCount` as an `attention` tray state — the same red flag a sync failure gets. New `packages/desktop/src/lib/secret-findings.js` (`deriveSecretFindings`/`openSecretFindings`/`secretFindingId`, pure and unit-tested) turns the local status endpoint's `secretScan.recentSuspected` into one dismissible finding per occurrence with a stable id; `main.js` wires this into the tray context menu (one submenu row per open finding: "Open in dashboard (rotate, don't redact)" opens the web dashboard, "Dismiss" clears only that finding) and a new `dismissSecretFinding` IPC handler exposed through `preload.cjs`. Dismissal is an in-memory, per-run acknowledgement (matching the tray's own ephemeral state), not a server-side mutation, so restarting the app or the agent shows a still-open finding again — the durable dismiss lives in the web dashboard's notification "Mark read".

Tests: `packages/agent/test/d1-backend.test.js` gained a round-trip assertion (`appendEvent('secret.suspected')` → `listNotifications` shows the right kind/title/body/href) inside the existing D1 collaboration end-to-end test. `src/lib/client/agent-status/mappers.test.ts` (3 tests), `src/components/features/activity/event-ledger.test.tsx` (2 tests), and `src/components/features/activity/notifications-card.test.tsx` (3 tests) are new vitest component/unit suites. `packages/desktop/test/secret-findings.test.js` (6 tests) is new; `packages/desktop/test/activity.test.js` and `packages/desktop/test/state.test.js` each gained assertions for the new `secret.suspected` describe-line and the `secretScan`-driven attention state respectively.

### 2026-07-24 GR-A4: unified reconciliation path + reconnect ordering

Closes the last gap in decisions §1's "one path for offline/crash/force-quit/restored-from-backup": until now, GR-A1's classification only ever saw paths that already had a pending journal entry, so an edit made while the agent was not running at all (offline, crashed, force-quit, or a workspace folder restored from an external backup) was invisible to reconnect classification entirely.

`watchWorkspace`'s startup path (`packages/agent/src/watch.js`) now runs a new `reconcileUnwatchedChanges` step before `recoverJournal`, fully awaited before it runs: it diff-scans the workspace against the workspace index's last-known content manifest (`diffWorkspaceAgainstManifest`), and for every added/modified/deleted path that does not already have an unresolved journal entry, synthesizes one (`synthesizeDiffScanEntries`, new in `packages/agent/src/reconnect.js`) and appends it to the journal. Because these land in the same journal file, `recoverJournal`'s existing GR-A1 pass classifies and replays/diverges them with no special-casing — exactly one recovery path for all four cases, never a second startup-only mechanism.

Restored-from-backup gets one deliberate exception: a deleted-path set shaped like a mass delete (reusing `refresh_would_mass_delete`'s `refreshMassDeleteMinFiles`/`refreshMassDeleteFraction` thresholds) is never trusted as a genuine local delete. Those entries are tagged `forceDivergence`, which `classifyReconnectEntry` now honors ahead of its normal both-touched check (when the cloud side still has the file): each one opens a divergence instead of replaying a delete that would wipe files still on cloud, so a workspace restored to an older snapshot never mass-deletes Main — it opens one divergence per missing file, and the local folder is left exactly as the restore left it until the user resolves via `hop conflicts resolve`.

Ordering invariant: `watchWorkspace` awaits `reconcileUnwatchedChanges`, then `recoverJournal`, then `hydrateWorkspace` in strict sequence, so the personal change set is always reconciled in full before a missed Main update is applied, never interleaved. Fixing this exposed a real clobber bug in the process: `hydrateWorkspace`'s disk-vs-cloud verify pass unconditionally overwrites any disk file that differs from cloud, which — without a skip list — would immediately clobber a path GR-A4 (or an earlier session) just opened as a divergence. `hydrateWorkspace` now accepts `options.hydrateSkipPaths` (`watchWorkspace` passes `recovery.divergedPaths`) so every currently-open divergence survives that pass untouched, mirroring GR-F1's `withheldPaths` pattern in `refreshWorkspace`.

Tests: `packages/agent/test/reconcile-unwatched-changes.test.js` — 11 new tests: 2 unit tests for `synthesizeDiffScanEntries` (baseline revision propagation, `forceDivergence` tagging only when mass-delete-shaped), 2 unit tests for `classifyReconnectEntry`'s `forceDivergence` handling (forces a divergence when cloud still has the file, falls through to the normal both-deleted auto-resolve when it does not), and 7 `watchWorkspace`/`reconcileUnwatchedChanges` integration tests covering: an offline edit journaled and acknowledged on restart, a brand-new offline file, a small offline deletion replaying as a normal delete, a genuine divergence between an offline edit and a Main change made while away, the restored-from-backup mass-delete guard (150-file fixture, 140 files missing on restart — asserts zero cloud deletions, 140 divergence records with `localEntry: null`, and the local folder left exactly as the restore left it), the reconcile-before-Main-update ordering invariant (asserted from `events.ndjson` event indices), and a no-op clean-restart case. `packages/agent/test/refresh-mass-delete-guard.test.js` (10/10) and `packages/agent/test/workspace-manifest-heal.test.js` — both left unmodified and green, confirming the invariants they encode were not weakened. Full battery: `npm run agent:test` 475 pass / 0 fail / 5 cancelled / 2 skipped (baseline 464/0/5/2 — 11 new tests, zero regressions), `npm run test:worker` 89/89, `npm run test:web` 226/226, lint/typecheck/build/CLI boot all clean.

### 2026-07-24 GR-B2: `hop propose` + merge-queue serialization

Implements the owner-approved `docs/proposal-data-model-design.md` (GR-B1) against real reads/writes (decisions §2-4): new `packages/backend-d1/src/proposals-store.js` (`attachProposalMethods`, mirroring `divergences-store.js`'s pattern exactly — `listProposals`/`getProposal`/`getOpenProposalForChangeSet`/`upsertProposal`/`approveProposal`/`markProposalStale`/`markProposalMerged`/`listReadyProposals`), plus the same method set added to `FixtureJsonCloudGraphService` (`packages/agent/src/cloud/d1-graph-service.js`) for the local/dev JSON backend, storing under a `proposals` array the same way `divergences`/`trailEpisodes` already do. No schema changes — GR-B1's table/migration already covers this.

New `packages/agent/src/commands/propose.js` is the command layer: `proposeChangeSet` (`hop propose [--title]`) pins the active change set's head, re-pinning the existing open (non-merged) proposal for that change set in place rather than forking a second row — the owner-set constraint ("at most one open proposal per change set," enforced at propose time in application code, since the GR-S1 drift-test parser only understands plain `create index` statements and can't express a partial unique index). `approveProposal` sets `state -> approved` and `queued_at` (the merge queue's FIFO key) — solo path: `hop propose --merge` (`proposeAndMerge`) pins, self-approves, then drains the queue through the exact same `runMergeQueue` path a team review-approval will use once GR-B3 wires the dashboard side. `runMergeQueue`/`landOneProposal` land every `approved` proposal oldest-first: direct land when Main is still at the proposal's recorded `base_revision`; otherwise a refresh check via `compareRevisions` (the existing WS7c diff engine, no new machinery) — this task's landing logic compares each proposal's *own* incremental paths from the just-refreshed Main position (not the original, now-stale `base_revision`) against what Main gained, because this codebase's single shared linear revision timeline means a later proposal's own range always structurally contains an earlier, already-landed proposal's changes as an ancestor; diffing from the stale base would flag every legitimate serial merge as a false conflict. No overlap lands cleanly; a genuine overlap leaves the proposal `stale`/`main_conflict` and the queue moves on, never auto-resolving.

`sync.js`'s `mergeChangeSet` (plain, proposal-less path — unchanged behavior, all its existing tests still pass) and the new queue now share one `advanceMainToRevision` primitive (extracted, move-only) for the actual `cloud.main` write + best-effort mirror-on-merge enqueue, so there remains exactly one code path that ever mutates `cloud.main.revision` (decisions §2: "Main has exactly one door"). Landing a proposal also rotates `cloud.selectedState` to a fresh `active-change-set` once its change set actually lands (it can never accept another save once merged — `selected_state_already_merged`) — the fresh change set's `baseRevision` is the just-landed Main revision, but its head starts at the live `cloud.revision`, so any unproposed saves made after the pin are carried forward, not discarded (verified by a dedicated test).

Not touched, by design: `cloudflare/d1/scoped-sql.js` (the CLI's D1 credential authenticates as the unrestricted proxy tier that bypasses this policy entirely, same as GR-A2's `divergences` table, which also isn't listed there); `review_decisions.decision_revision`/`.proposal_id` writes and the stale-on-re-pin automation (GR-B3, depends on this task).

Tests: new `packages/agent/test/propose.test.js` — 16 tests: propose pins the active head; propose alone never touches Main; re-proposing the same change set re-pins in place (not a fork); title preserved across a re-pin, explicitly clearable; `propose --merge` solo self-approve+land in one command, including proof the rotated change set accepts further saves; saves after proposing don't change what lands; two ready proposals (seeded at the store layer, the way GR-A2's tests seed `openDivergence`, since a single live `selectedState` can't itself produce two simultaneously-approved proposals) merge serially with the second refreshed against the first's landed result, asserted against final Main content via `compareRevisions`; a second merge attempt explicitly observes the first's already-advanced Main revision (`previousMainRevision`) — the concurrency-serialization metric; a genuine same-path overlap refuses to merge and leaves `stale`/`main_conflict`; approve-with-no-proposal and approve-already-merged both reject; propose rejects once the change set merged via the plain path; the plain `hop merge` path still works unchanged and never touches `proposals`; two CLI end-to-end tests (`--title`/`--merge` wiring, and pin-only emits no approve/merge events); one D1-backend parity test (`propose --merge` round-trips through a real in-memory-sqlite-backed `proposals` table row and lands via `CloudflareD1HopBackend`). Full battery: `npm run agent:test` 480 pass / 0 fail / 2 skipped / 5 cancelled (same 5 environment-only cancellations as baseline, all pre-existing); `npm run test:worker` 89/89; `npm run test:web` 226/226; `npm run lint`, `npm run typecheck:agent`, `npm run build`, `node packages/agent/src/cli.js --help` all clean.

### 2026-07-24 GR-B3: stale-review automation

Implements decisions §4's "the proposer explicitly updates the proposal, which automatically marks existing reviews as stale" against GR-B1's `review_decisions.decision_revision`/`.proposal_id` columns and GR-B2's `proposals`/merge-queue machinery (both previously schema/plumbing-only for this purpose). `createReviewDecision` (`packages/backend-d1/src/collaboration.js`) now looks up the change set's open (non-merged) proposal and writes `decision_revision = proposal.pinnedRevision` / `proposal_id = proposal.proposalId` on every decision row (both stay `NULL` for decisions that predate proposals); an `approved` decision against that proposal calls the same `approveProposal` (`state -> approved`, `queued_at` set) the solo `hop propose --merge` self-approve path calls directly — this is the team review-approval door into the merge queue decisions §2/§3 describe, now actually wired. New `decorateReviewDecisionsWithStaleness` computes staleness on read (never by mutating the append-only decision row) by joining each decision's `decisionRevision` against its proposal's *live* `pinned_revision`; `listReviewDecisions`/`createReviewDecision` both return the decorated shape (`currentPinnedRevision`, `stale`).

The merge queue (`landOneProposal` in `packages/agent/src/commands/propose.js`) adds an explicit guard: before landing an `approved` proposal, it asks the backend for the most recent `review_decisions` row linked to that `proposal_id` (new `getLatestDecisionForProposal`, on both `packages/backend-d1/src/proposals-store.js` for the real D1 backend and a `null`-returning stub on the fixture/local-dev graph service, since that backend has no `review_decisions` table at all). No linked decision means solo self-approve — never blocked, GR-B2's path stays intact. A linked decision must be the most recent one recorded, `approved`, and still pinned at the proposal's current `pinned_revision`, or the queue marks the proposal `stale`/`stale_reason: 'review_stale'` and moves to the next one — this catches not just "re-pinned after approval" (already structurally blocked by `upsertProposal`'s state reset, GR-B2) but also "a later `changes-requested` decision superseded an earlier approval at the same pin," which the state reset alone would miss.

Dashboard: `src/components/features/review/decisions-card.tsx` renders a "changed since review" badge on a stale decision and lazily fetches a file-count summary via the existing `/api/codebases/compare` route (`compareRevisions(reviewedRev, pinnedRev)` — no new diff machinery, per decisions §4's own instruction). `src/lib/collaboration.ts`'s `ReviewDecision` type gains `proposalId`/`decisionRevision`/`currentPinnedRevision`/`stale`; `use-review-data.ts` exposes `codebaseId` so the card can build compare requests. No new proposals-facing API route was added — team-facing UI stays behind the collaboration-surface freeze (Track B, per the plan) beyond this existing-surface enhancement.

No schema changes: GR-B1 already shipped the `decision_revision`/`proposal_id` columns this task writes to.

Tests: new `packages/agent/test/stale-review-automation.test.js` (5 tests, all against a real in-memory-sqlite-backed D1 worker since the fixture backend has no review-decision concept) — an approved team decision links to the proposal and lands via the merge queue; re-pinning after approval flags the prior decision stale (`currentPinnedRevision` moves, `decisionRevision` doesn't) and blocks the queue until re-approval; the queue guard explicitly refuses to land when the latest linked decision is a `changes-requested` at the same pin (proposal state alone still says `approved`); the solo self-approve path creates no linked decision and is never blocked; a decision recorded before any proposal exists stays unlinked and never stale. New `src/components/features/review/decisions-card.test.tsx` (3 tests): no badge for a non-stale decision, badge + fetched file-count summary for a stale one, no badge for a proposal-less (pre-proposals) decision. Full battery: `npm run agent:test` — baseline 491 pass / 0 fail / 2 skipped / 5 cancelled, this task adds 5 more passing tests (496 pass / 0 fail / 2 skipped / 5 cancelled, same 5 environment-only cancellations as baseline); `npm run test:worker` 89/89 (unchanged); `npm run test:web` 237/237 (234 baseline + 3 new); `npm run lint`, `npm run typecheck:agent`, `npx tsc --noEmit -p tsconfig.json`, `node packages/agent/src/cli.js --help` all clean.

### 2026-07-24 GR-B5: CI on propose (actions-runner hardening)

Implements decisions §3 ("CI's default trigger is on propose and in the merge queue, not on every save") against the previously-untested `packages/actions-runner/src/runner.js` seed, and wires it into GR-B2's propose/merge-queue path.

Schema: `action_jobs` gains an additive nullable `proposal_id` column (`alter table`, matching the existing `review_decisions.proposal_id` pattern) plus `idx_action_jobs_proposal` — `packages/backend-d1/src/schema.js`, `cloudflare/d1/schema.sql`, `cloudflare/d1/migrations/2026-07-24-action-jobs-proposal-link.sql` (never applied automatically). `helpers/actions.js` gains a `ci` job kind (`npm test`, same command a hosted `test` job runs) and carries `proposalId` through `summarizeActionJob`. New backend methods `enqueueCiJobForProposal`/`getLatestCiJobForProposal` in `packages/backend-d1/src/actions.js`, mirrored as deliberate no-ops on the local/dev fixture backend (`packages/agent/src/cloud/d1-graph-service.js`) — same rationale as GR-E2's `enqueueMirrorSyncJob`: there is no hosted runner or `action_jobs` table in that mode, so CI only actually gates the merge queue when `cloudService.type === d1CloudServiceType`.

`packages/agent/src/commands/propose.js`: `proposeChangeSet` best-effort enqueues a `kind: 'ci'` job tied to the new proposal right after pinning (an enqueue failure never blocks the pin, only emits `ci.enqueue_failed`). `landOneProposal` now re-checks the proposal's latest CI job immediately before it would land (never trusts an earlier answer): `succeeded` proceeds to the existing overlap/staleness check; `queued`/`running` returns `ci-pending`; missing or `failed` (re-)enqueues a fresh job and returns `ci-pending`/`ci-failed` — a failed check never auto-passes, only a fresh green job unblocks the proposal. `runMergeQueue`'s drain loop now also stops on `ci-pending`/`ci-failed` (previously only `skipped`), since the queue is strictly serial/FIFO and a blocked head-of-queue proposal must never let a later one land out of order. This changes `hop propose --merge`'s solo behavior: it still pins and self-approves in one command, but the drain right after self-approval typically returns `ci-pending` (nothing has run the check yet) rather than landing synchronously — a later `runMergeQueue` call, once a runner reports success, actually lands it. `packages/agent/test/propose.test.js`'s existing D1 round-trip test was updated for this (claim/complete the CI job via the backend, exactly like a hosted runner would, before re-draining), and a new test proves red CI blocks the land (Main untouched), a failed job gets re-queued rather than retried in place, and a fresh green job then lands it.

`packages/actions-runner/src/runner.js` is split into a dependency-injected `createActionsRunner(config)` factory (backend, spawn/sleep functions, clocks all overridable) plus a guarded CLI entrypoint, so the module can be imported for tests without starting the poll loop or throwing on a missing cloud backend. Behavior preserved: claim -> hydrate -> (for non-`mirror` kinds) `npm install --ignore-scripts`-if-needed -> run the job command -> complete; `mirror` kind still skips straight to `mirror-sync` (GR-E2, no hydrate needed). New: claim errors (transient D1/network trouble — `claimNextActionJob` returning `null` for "no work" is not an error) retry with exponential backoff (`claimRetry: { maxAttempts, baseDelayMs, maxDelayMs }`); an exhausted retry cycle is logged and the poll loop continues rather than crashing the runner process. The job *step's* environment (`actionJobEnv`) remains built from `safeBaseEnv` only — no `HOPIT_D1_*`/`CLOUDFLARE_*`/`HOPIT_R2_*`/etc — while the runner's own trusted steps (hydrate, mirror-sync) still get `trustedAgentEnv`; this separation already existed in the seed and is now covered by tests instead of only by code review.

Tests: new `packages/agent/test/actions-runner.test.js` — 9 tests (target was ≥ 8): a full claim -> hydrate -> run -> complete success cycle against a real loopback D1 backend (proves the workspace was actually hydrated from the fixture, not stubbed); a command-failure path (non-zero exit captured with stderr); a timeout path (a hung child process is killed and reported failed); output capping (a large stdout is truncated to the configured byte budget, keeping the tail); env lockdown as a unit test (`actionJobEnv` excludes every `HOPIT_D1_*`/`CLOUDFLARE_*`/`HOPIT_R2_*`/`HOPIT_CLIENT_ENCRYPTION_*` key while `trustedAgentEnv` includes them) and end-to-end (a real spawned job step's `process.env`, captured via `JSON.stringify(process.env)`, is asserted to contain zero `HOPIT_D1_*`/`CLOUDFLARE_*` keys); claim retry/backoff succeeding after transient errors (asserts the exact doubling delay sequence) and exhausting all attempts (asserts it surfaces the last error rather than hanging); the poll loop surviving an exhausted claim-retry cycle and continuing to poll. Plus 2 new tests in `propose.test.js` (queue-blocks-on-red-CI scenario, and the updated D1 round-trip). `packages/agent/test/schema-drift.test.js` still passes (proves `schema.sql`/`schema.js` didn't drift).

Full battery on `gr/b5`: `npm run agent:test` 508 tests, 501 pass / 0 fail / 2 skipped / 5 cancelled (the same 5 environment-only cancellations as the documented baseline — periodic head reconciliation x2, reconnect-fallback-poll-with-open-socket, initial-push-state-recovers-cursor, genuine-no-socket-fallback-poll — all pre-existing, not caused by this change); baseline was 498 tests / 491 pass, so this is +10 tests, all new, zero regressions. `npm run test:worker` 89/89 (unchanged). `npm run test:web` 234/234 (unchanged). `npm run lint`, `npm run typecheck:agent`, `npm run build`, `node packages/agent/src/cli.js --help` all clean.

Out of scope, reported not fixed: containerization of job execution (explicitly deferred per the plan); a dashboard/CLI surface to show CI status on a proposal (no such surface existed before this task and none was requested); GR-B3's `review_decisions.decision_revision`/`.proposal_id` writes and stale-on-re-pin automation (unrelated, separate task).

### 2026-07-24 GR-B4: Releases

A lightweight, permanent pin of a Main revision (decisions §9: "mark this state as a release" — name, optional notes, pinned Main revision, `created_at`, duplicate names rejected — without a git tag in the product). New `packages/backend-d1/src/releases-store.js` (`attachReleaseMethods`, same shape as `proposals-store.js`/`divergences-store.js`): `listReleases`/`getRelease`/`getReleaseByName`/`createRelease`, against a new `releases` table (`release_id`, `codebase_id`, `name`, `notes`, `pinned_revision`, `created_by_user_id`, `created_at`; schema/index additions in `packages/backend-d1/src/schema.js` and `cloudflare/d1/schema.sql`, dated migration `cloudflare/d1/migrations/2026-07-24-releases.sql`, never applied automatically). Duplicate names per codebase are rejected in application code (`createRelease` checks `getReleaseByName` first, throws `DuplicateReleaseNameError`) rather than a SQL unique constraint — same reasoning as GR-B1/B2's open-proposal constraint: the GR-S1 drift-test parser only understands plain `create index` statements. The same method set is mirrored onto `FixtureJsonCloudGraphService` (`packages/agent/src/cloud/d1-graph-service.js`) for the local/dev JSON backend, storing under a `releases` array the same way `proposals`/`divergences` already do.

New `packages/agent/src/commands/release.js` is the CLI surface: `hop release <name> [--notes <text>]` (`createRelease`) pins the codebase's *current* `cloud.main.revision` — deliberately not the caller's active change set head, since a release always describes what is actually live on Main right now. Unlike proposals, every call inserts a brand-new row; releases are never re-pinned or mutated once created (they are historical facts, "what shipped as v1.2"). `hop release list` reports releases newest-first. Wired into `cli.js` alongside `propose` (same raw-event/JSON output convention, not human-summarized by default, matching how `propose` tests assert on raw stdout).

Dashboard: read-only `ReleasesCard` (`src/components/features/files/releases-card.tsx`) plus its data hook (`use-releases.ts`) and a thin `/api/codebases/releases` route (`src/app/api/codebases/releases/route.ts`, mirroring the derived-paths route's auth-then-read shape: a `compareRevisions` probe authorizes the read, then `listReleases`). Rendered on the codebase page (`files-page.tsx`) behind `NEXT_PUBLIC_RELEASES_LIST`, following GR-C1's collaboration-surface-freeze convention (Track B's dashboard surfaces stay flag-gated until Phase 4; `hop release` itself is unaffected by the flag).

Not implemented, by design: git-tag emission on the mirror is GR-E3 (a later task); this table's `name`/`pinned_revision` are shaped for E3 to read directly (tag name candidate, revision to build the git tree from) without a further schema change.

Tests: new `packages/agent/test/release.test.js` — 9 tests: release pins the *current* Main revision, not the active change set head; releasing pins the exact Main revision — the compare engine (`compareRevisions`) shows zero diff between the release's `pinnedRevision` and the Main revision actually captured at creation time (the metric); create/list round-trip (newest-first ordering, `getRelease`/`getReleaseByName`); duplicate names rejected per codebase, with proof the rejected call left no second row; a name is required; two CLI end-to-end tests (`hop release <name> --notes` pins and persists notes, `hop release list --json` reports the list); `runReleaseCommand` dispatch parity with the CLI's action parsing; one D1-backend parity test (release round-trips through a real in-memory-sqlite-backed `releases` table row, including the duplicate-name rejection, via `CloudflareD1HopBackend`). New `src/app/api/codebases/releases/route.test.ts` — 8 tests covering the route's request/response mapping (codebaseId required, backend-unavailable/D1-required, auth failure/missing-auth, the authorize-then-read happy path, notes defaulting, read failure). New `src/components/features/files/releases-card.test.tsx` — 5 tests (loading, empty, single release with notes/revision, multiple releases, error) — the dashboard render-test metric. `packages/agent/test/schema-drift.test.js` re-run clean (releases table/indexes agree between `schema.js` and `schema.sql`). Full battery: `npm run agent:test` 507 tests / 500 pass / 0 fail / 5 cancelled / 2 skipped (baseline 498/491/0/5/2 — 9 new tests, zero regressions, same 5 environment-only cancellations as baseline); `npm run test:worker` 89/89 (unchanged); `npm run test:web` 247 pass / 0 fail (baseline 234 — 13 new tests, zero regressions); `npm run lint`, `npm run typecheck:agent`, `npm run build`, `node packages/agent/src/cli.js --help` all clean.

### 2026-07-24 GR-E3: Releases → git tags

Closes the loop between GR-B4 (releases) and GR-E1/E2 (the continuous git mirror): `hop release <name> [--notes]` now also pushes an annotated git tag to the mirror when one is configured for the codebase (decisions §8/§9 — "mark this as a release" ⇒ "emitted as a git tag on the mirror").

Every mirror commit built by `hop mirror-sync` (`packages/agent/src/commands/mirror.js`, `commitMessageForRevision`) now carries a `Main-Revision: <n>` git trailer alongside its existing subject line. This is the durable, remote-verifiable way to answer "which mirror commit is Main revision N at?" without any per-machine local bookkeeping — the pushed git history itself is the source of truth, consistent with the mirror's own "one-way, deterministic" design. New `runMirrorTagRelease(options, { cloudService, cloud, release })`, called from `createRelease` (`packages/agent/src/commands/release.js`) right after the release row is inserted: resolves the configured mirror remote/branch (same `--remote`/local mirror-state/`codebase_settings` fallback chain `mirror-sync` uses), fetches the mirror branch, finds the commit whose `Main-Revision` trailer matches the release's `pinnedRevision` (`git log --format=%(trailers:key=Main-Revision,valueonly)`, git's own trailer parser — never hand-rolled message string-matching, which would also risk `Main-Revision: 1` false-matching `Main-Revision: 10`), and pushes an annotated tag named after the release with `name\n\nnotes` (or just `name` when there are no notes) as the tag message. Tag names are validated by delegating to git's own `check-ref-format --allow-onelevel` (new `assertSafeGitTagName`) rather than re-implementing the ref-name grammar, mirroring how `validateGitRemoteUrl` already leans on the URL parser; combined with a `--` separator before the tag name when it is passed to `git tag`, this also rules out flag-injection via a release name starting with `-`.

Layered the same way GR-E2 layers mirror-on-merge: no mirror configured at all for the codebase ⇒ `runMirrorTagRelease` returns `null` silently (a release is meaningful without a mirror) and `release.mirrorTag` is `null`. A mirror that *is* configured but hasn't caught up to the release's pinned revision yet (or any other push/auth failure) ⇒ the release itself still succeeds (never blocked, never rolled back) and a `git.mirror_tag_failed` event carries the error, same "surfaces as a notification, never blocks the product action" rule as a mirror-on-merge failure. `hop release`'s CLI report additionally prints `✓ Tagged mirror <name> (<sha>)` when tagging succeeded.

Tests: new `packages/agent/test/mirror-release-tag.test.js` — 4 tests against a local bare repo as the mirror remote: (1) no mirror configured — `mirrorTag: null`, no `git.mirror_tag_failed` event; (2) mirror configured and caught up — the tag lands on the exact mirror commit for the pinned revision, the tag message carries the release notes (asserted via `git tag -l --format=%(contents)` on the bare remote), and **the GR-E3 metric**: the tagged commit's tree hash equals the tree hash independently captured from the bare remote right after `mirror-sync` ran (before tagging), proving the tag landed on the correct commit's tree, not just a same-named variable; (3) release created before the mirror has synced its revision — release still succeeds, `git.mirror_tag_failed` fires with a `mirror-sync`-mentioning error, and no tag lands on the remote; (4) `assertSafeGitTagName` unit coverage (space, `..`, empty string rejected; a normal name and a `-`-prefixed name both accepted, since the `--` separator — not the validator — is what neutralizes the flag-injection risk of a leading dash). Full battery: `npm run agent:test` 526 tests / 519 pass / 0 fail / 5 cancelled / 2 skipped, run twice for consistency (baseline post-Wave-4 522/515/0/5/2 — 4 new tests, zero regressions, same 5 environment-only cancellations/2 skips as baseline); `npm run test:worker` 89/89 (unchanged); `npm run test:web` 250 pass / 0 fail (unchanged, no web surface touched by this task); `npm run lint`, `npm run typecheck:agent`, `npm run build`, `node packages/agent/src/cli.js --help` all clean.

Out of scope, reported not fixed: a `hop mirror-tag <release-name>` standalone CLI verb for re-tagging without creating a new release (the plan's Do/Accept only requires tagging as a side effect of `hop release`); retagging/re-pushing a tag if the same release name's tag is later found missing on the remote (currently only happens once, at `hop release` time); a dashboard surface showing tag status per release (no such surface was requested and Track B's release dashboard card is itself still behind `NEXT_PUBLIC_RELEASES_LIST`).
