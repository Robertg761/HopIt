...
- `hop setup` now provides a streamlined interactive first-run flow: it asks permission to open the system folder picker, accepts a chosen Workspace Root, requires an explicit cloud-upload/local-removal warning confirmation for non-empty roots, and seeds local agent state/workspace/index plus a pre-filled production env file with safe defaults. `--advanced`, `--yes`, and explicit flags preserve lower-level and scripted configuration. Start-on-login still relies on the script/template service plumbing, and the operator must still fill real credential values into the local env file.
