...
- The production push hub is deployed and the installed agent has push enabled. The long-standing push skip was diagnosed on 2026-07-10 as a stale content manifest rather than real drift and healed, so the workspace is scan-clean; a clean-workspace live `push-applied` proof remains an explicit gap.
