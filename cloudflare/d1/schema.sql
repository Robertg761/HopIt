...
-- after that accumulate as "since proposal" against the same `change_set_id`
-- without moving the pin. Re-pinning (the proposer explicitly updating the
-- proposal) advances `pinned_revision` in place on this same row and resets
-- `state` to 'proposed', which is what makes prior
-- `review_decisions.decision_revision` values stale. `state` is one of
-- draft/proposed/approved/stale/merged (decisions §4; 'draft' is reserved for
-- a future save-as-you-go flow, unused by `hop propose` in v1). `queued_at`
-- is set when `state` becomes 'approved' and is the merge queue's FIFO
-- ordering key; merges are serialized by the codebase's existing `revision`
-- compare-and-swap (see design doc §"Merge queue serialization"), not a new
-- queue table. `base_revision` is Main's revision as of the pin, used to
-- detect whether a refresh is needed before landing. See
-- docs/proposal-data-model-design.md for the full state machine and
-- traceability table.
...
-- the divergence is open -- the local workspace copy is the only other place
-- it lives, and it must never be clobbered before the user resolves. The
