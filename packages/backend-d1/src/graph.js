...
export function attachGraphMethods(Backend) {
  ...
  async appendEvent({ codebaseId = this.codebaseId, event, detail, at = new Date().toISOString(), source = 'local-agent' }) {
    ...
    // (GR-D1) never blocks the upload; the dashboard/menu-bar flag is this
    // notification round-trip, fired the moment the event lands so it shows
    // up on the next poll/push cycle regardless of which surface asks first.
    if (event === 'secret.suspected') {
      await this.createNotification({
        codebaseId,
        kind: 'secret.suspected',
        title: secretFindingTitle(detail?.path),
        body: secretFindingBody(detail?.path, detail?.findingCount),
        href: secretFindingHref(codebaseId, detail?.path),
        createdAt: at,
      })
    }
  },
