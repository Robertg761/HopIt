...
async function landOneProposal(options, cloudService, candidate, actorId) {
  ...
  // GR-B5 CI gate (decisions §3): runs after the stale-review guard -- a
  // stale approval definitively flips the proposal's state, so there is no
  // point holding the queue on a CI check for it. Both guards must pass
  // before a proposal lands.
  if (ciGatingEnabled(cloudService)) {
