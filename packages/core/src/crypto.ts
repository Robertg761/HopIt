...
import {
  ...
  privacyZoneForPath,
  privacyZoneIdForPath,
  privacyZoneKind,
} from './privacy-zone.js'
...
export function clientEncryptionScopeFromOptions(options = {}, env = process.env) {
  ...
  if (raw === 'private') return clientEncryptionScopes.ownerPrivate
  if (raw === clientEncryptionScopes.ownerPrivate) return clientEncryptionScopes.ownerPrivate
  if (raw === clientEncryptionScopes.all) return clientEncryptionScopes.all
  if (raw === clientEncryptionScopes.off || raw === '0' || raw === 'false') return clientEncryptionScopes.off
...
export function shouldEncryptWithConfig(relativePath, config) {
  if (!config || config.scope === clientEncryptionScopes.off) return false
  if (config.scope === clientEncryptionScopes.all) return true
  if (config.scope === clientEncryptionScopes.ownerPrivate) return hasPrivatePrivacyZone(relativePath)
...
export function decryptClientPayload({ buffer, codebaseId, relativePath, encryption, config }) {
  if (!encryption || encryption.state !== clientEncryption.state) return buffer
  if (!config) {
...
export function normalizeClientEncryptionMetadata(value) {
  ...
  if (value.state !== clientEncryption.state) return null
  return {
...
export function validateClientEncryptionMetadata(metadata, label = 'clientEncryption') {
  const errors = []
  if (!metadata || metadata.state !== clientEncryption.state) return errors
  if (metadata.version !== undefined && metadata.version !== clientEncryption.version) errors.push(`${label}.version is invalid.`)
  if (metadata.algorithm !== clientEncryption.algorithm) errors.push(`${label}.algorithm is invalid.`)
  if (metadata.aadVersion !== undefined && metadata.aadVersion !== clientEncryption.aadVersion) errors.push(`${label}.aadVersion is invalid.`)
  ...
  if (!Number.isInteger(metadata.plaintextSize)) errors.push(`${label}.plaintextSize is required.`)
  if (metadata.zone !== undefined && !Object.values(privacyZoneKind).includes(metadata.zone)) errors.push(`${label}.zone is invalid.`)
  if (metadata.zoneId !== undefined && !isNonEmptyString(metadata.zoneId)) errors.push(`${label}.zoneId is invalid.`)
  if (metadata.ciphertextHash !== undefined && !isNonEmptyString(metadata.ciphertextHash)) errors.push(`${label}.ciphertextHash is invalid.`)
  if (metadata.ciphertextSize !== undefined && !Number.isInteger(metadata.ciphertextSize)) errors.push(`${label}.ciphertextSize is invalid.`)
  return errors
}
...
export function encryptRecoveryPayload({ key, passphrase, context }) {
  if (!passphrase) throw new Error('Recovery export requires a passphrase.')
  const salt = randomBytes(16)
  const nonce = randomBytes(clientEncryption.nonceBytes)
...
export function decryptRecoveryPayload({ recovery, passphrase, context }) {
  ...
  if (recovery.context !== context) throw new Error(`recovery_context_mismatch: expected ${context}`)
